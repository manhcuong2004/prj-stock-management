...
def product_category_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            message = f"{username} đã tạo danh mục {category.category_name} thành công!"
...
def product_category_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            category = form.save()
            username = request.user.username
            message = f"{username} đã cập nhật danh mục {category.category_name} thành công!"
...
def product_category_delete(request, pk):
    ...
    if request.method == "POST":
        if Product.objects.filter(category=category).exists():
            ...
            category.delete()
            username = request.user.username
            message = f"{username} đã xóa danh mục {category_name} thành công!"
