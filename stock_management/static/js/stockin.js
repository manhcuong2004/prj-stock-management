...
// Hàm làm mới product-select và product-batch
...
// Xử lý sự kiện khi thay đổi sản phẩm hoặc mã lô
    ...
    if (productId) {
        ...
        newRow.innerHTML = `
            ...
            <td class="text-center">
                <input type="checkbox" name="stockindetail_set-${formIndex}-DELETE" id="stockindetail_set-${formIndex}-DELETE" class="delete-checkbox">
...
document.addEventListener('click', function(event) {
    if (event.target.classList.contains('delete-row')) {
        if (confirm('Bạn có chắc muốn xóa sản phẩm này?')) {
            ...
            if (!deleteInput) {
                console.error('Delete input not found for row:', row);
                return;
            }

            if (idInput && idInput.value) {
                // Hàng đã tồn tại trong DB, đánh dấu DELETE và ẩn
                deleteInput.checked = true;
                row.style.display = 'none';
                console.log('Marked DELETE for row with id:', idInput.value);
            } else {
                // Hàng mới, xóa hoàn toàn khỏi DOM
                row.remove();
                const totalForms = document.querySelector('#id_stockindetail_set-TOTAL_FORMS');
                totalForms.value = parseInt(totalForms.value) - 1;
                formIndex--;
                console.log('Removed new row, TOTAL_FORMS:', totalForms.value);
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.product-row').forEach(row => {
        ...
        if (batchInput && batchInput.value) {
            batchInput.setAttribute('value', batchInput.value); // Đảm bảo giá trị hiển thị
        } else if (batchInput) {
            console.log('Batch input is empty:', batchInput.name); // Debug nếu input trống
        }
    });
});
